"""
Evaluation script: Vary Total Dataset Size — Full LPM (OLS)

Computes Euclidean sRMSE over the full 6-vector θ = [β₀, β₁, β₂, β₃, β₄, β₅].
...
if __name__ == "__main__":
    ...
    srmse, srmse_se, n_valid = compute_metrics_euclidean(llm_betas, beta_star)
    bias, bias_se, _         = compute_bias_euclidean(llm_betas, beta_star)
    rows.append({
        "dataset": args.dataset, "llm": args.llm, "method": "llm_only",
        "n_expert": n_expert, "N_total": None,
        "sRMSE_eucl": round(srmse, 6), "sRMSE_eucl_se": round(srmse_se, 6),
        "bias_eucl":  round(bias,  6), "bias_eucl_se":  round(bias_se,  6),
    ...
    for method_name, all_thetas in methods.items():
        for i, N in enumerate(N_values):
            ...
            srmse, srmse_se, n_valid = compute_metrics_euclidean(betas_at_N, beta_star)
            bias, bias_se, _         = compute_bias_euclidean(betas_at_N, beta_star)
            rows.append({
                "dataset": args.dataset, "llm": args.llm, "method": method_name,
                "n_expert": n_expert, "N_total": int(N),
                "sRMSE_eucl": round(srmse, 6), "sRMSE_eucl_se": round(srmse_se, 6),
                "bias_eucl":  round(bias,  6), "bias_eucl_se":  round(bias_se,  6),
                "n_reps": num_reps, "n_valid": n_valid,
            })
