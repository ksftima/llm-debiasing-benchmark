"""
Experiment: Vary Expert Sample Size — Phase 4: Full Logistic Regression (Unregularized)

Identical to expert_full_logistic.py but runs all methods without L2 regularization:
...
Results are saved to vary-expert-full-logistic-noreg/<dataset>/<llm>/rep_<seed>.npz.
"""
...
def fit_ppi_full_noreg(Y, Y_hat, X, selected_mask):
    """
...
def fit_dsl_full_noreg(Y, Y_hat, X_df, selected_mask, ro):
    """
    DSL estimate for full logistic regression with lambda=0 (unregularized).
...
def compute_one_n(Y, Y_hat, X, X_df, n, seed, ro):
    ...
    beta_exp   = fit_logistic_full_unregularized(Y[selected_mask], X[selected_mask])
    beta_dsl   = fit_dsl_full_noreg(Y, Y_hat, X_df, selected_mask, ro)
    beta_ppi   = fit_ppi_full_noreg(Y, Y_hat, X, selected_mask)
    beta_ppipp = fit_ppipp(Y, Y_hat, X, selected_mask, lam_l2=0.0)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--n-select", type=int, nargs="+", default=None,
        help="Run only at these n values instead of the full log-spaced grid")
    args = parser.parse_args()

    print(f"Seed: {args.seed} | CSV: {args.annotated_csv} | mode: unregularized")
