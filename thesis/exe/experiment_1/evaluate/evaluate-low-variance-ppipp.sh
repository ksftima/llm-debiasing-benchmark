#!/bin/bash
#
# Evaluate low-variance PPI++ (Phase 2) results for all LLMs.
# Outputs one ppipp-only CSV per LLM — merge with originals locally.
#
# Usage: bash evaluate-low-variance-ppipp.sh <dataset>
# Example: bash evaluate-low-variance-ppipp.sh cuad
...
for LLM in llama deepseek gpt54 mistral claude; do
    RESULTS_DIR="/code/thesis/results/vary-expert-low-variance-ppipp/${DATASET}/${LLM}"
    OUTPUT_CSV="/code/thesis/results/summaries-ppipp/${DATASET}_${LLM}_low_variance_ppipp.csv"
